	...
	createClass(Alert, [{
		...
		value: function componentDidUpdate() {
			...
			if (Array.isArray(updateResponse)) {
				updateResponse.forEach(function (elem) {
					if (elem.error) {
						_this2.showErrorAlert(elem.error);
					}
					if (elem.success || elem.status) {
						_this2.showSuccessAlert(elem.message);
					}
					_this2.props.clearResponse.forEach(function (clear) {
						clear();
					});
				});
			}
...
Alert.propTypes = {
	updateResponse: propTypes.oneOfType([propTypes.object, propTypes.array]),
